...
dataRoot = r'/mnt/HDD1/zhenghong/Viewer/MRIcroGL_Joon/data1/'
...
# Assuming zCT always exists based on your prompt, but you could wrap this too if needed
...
# --- yCT Processing (Conditional) ---
yCTDataDir = os.path.join(dataRoot, 'RPA_yCT', 'SUV')
yCTResultDir = os.path.join(dataRoot, 'RPA-result-yct-right_lobe3', 'processed_mask')
yCTpairs = [] # Initialize as empty so code doesn't break if skipped

if os.path.exists(yCTDataDir):
    yCTSUVFiles = sorted(os.path.join(yCTDataDir, f) for f in os.listdir(yCTDataDir) if f.endswith('.nii.gz'))
    print("yCT SUV files found:", len(yCTSUVFiles))

    for yCTFile in yCTSUVFiles:
        baseName = os.path.basename(yCTFile).replace('.nii.gz', '')
        caseResultDir = os.path.join(yCTResultDir, baseName)
        caseResultFiles = [os.path.join(caseResultDir, f) for f in roiMaskFileNames]
        caseResultFiles.insert(0, yCTFile)
        yCTpairs.append(caseResultFiles)
    print("yCT pairs prepared:", len(yCTpairs))
else:
    print("yCT data directory not found. Skipping yCT.")
