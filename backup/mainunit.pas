...
uses
  ...
  lcltype, GraphType, Graphics, dcm_load, crop, intensityfilter, LCLIntf,
  slices2D, StdCtrls, SimdUtils, Classes, SysUtils, Forms, Controls, clipbrd,
  Dialogs, Menus, ExtCtrls, CheckLst, ComCtrls, Spin, Types, fileutil,
  ulandmarks, nifti_types, nifti_hdr_view, fsl_calls, math, nifti, niftis,
  prefs, dcm2nii, strutils, drawVolume, autoroi, VectorMath, LMessages, Buttons;
...
type
  ...
  TPatientEntry = class
    ID: string;
    CTPath: string; // 存储 CT 或者 默认图像
    PTPath: string; // 存储 PET 或者 Overlay 图像
  end;
  ...
  TGLForm1 = class(TForm)
    AnatDrop: TComboBox;
    BtnClearCommentHist: TToggleBox;
    BtnClearScoreHist: TButton;
    BtnCommentHistory: TButton;
    BtnDeleteScore: TButton;
    BtnHistory: TButton;
    BtnJumpTo: TButton;
    BtnNextPatient: TButton;
    BtnPrevPatient: TButton;
    BtnSaveComment: TButton;
    BtnSaveScore: TButton;
    BtnLoadMasks: TButton;
    CenterPanel: TPanel;
    CheckIsScored: TCheckBox;
    ClipAziLabel: TLabel;
    ...
    MPRLeftNoseMenu: TMenuItem;
    MPR4Menu: TMenuItem;
    QLabel: TLabel;
    QualityTrack: TTrackBar;
    RadioScore1: TRadioButton;
    RadioScore2: TRadioButton;
    RadioScore3: TRadioButton;
    RadioScore4: TRadioButton;
    RadioScore5: TRadioButton;
    ...
    procedure BtnClearCommentHistChange(Sender: TObject);
    procedure BtnClearScoreHistClick(Sender: TObject);
    procedure BtnCommentHistoryClick(Sender: TObject);
    procedure BtnDeleteScoreClick(Sender: TObject);
    procedure BtnHistoryClick(Sender: TObject);
    procedure BtnJumpToClick(Sender: TObject);
    procedure BtnLoadMasksClick(Sender: TObject); // 绑定这个事件
    procedure BtnNextPatientClick(Sender: TObject);
    procedure BtnPrevPatientClick(Sender: TObject);
    ...
    procedure CheckAutoRunPyChange(Sender: TObject);
    ...
    procedure MosRowLabelClick(Sender: TObject);
    procedure NimlMenuClick(Sender: TObject);
    procedure RadioScore2Change(Sender: TObject);
    ...
    procedure TimerFixPropsTimer(Sender: TObject);
  ...
  private
    FPatientList: TList; // 存 PT 文件路径
    FCurrentIndex: integer;   // 当前索引 (从 0 开始)
    FDataPath: string;
    FLogPath: string;
    procedure InitFileLists;  // 初始化文件列表的函数
    procedure LoadCurrentPair; // 加载当前索引图像的函数
    function GetPatientID(vol: TNIfTI): string;
    procedure LoadScoreFromCSV;
    procedure SaveScoreToCSV;
    procedure UpdateScoreUI(score: integer); // 辅助更新界面状态
    procedure DeleteScoreFromCSV;
    procedure ShowHistoryLog; // 【新增】显示历史
    procedure ShowCommentHistory; // 【新增】显示评论历史
    procedure DeleteHistoryLog(logFilePath: string; logDesc: string);
    procedure ClearPatientList;  // 新增：清理内存
    // --- 路径获取 ---
    function GetPath_ScoreCurrent: string;      // scores.csv
    function GetPath_ScoreHistory: string;      // score_log.csv
    function GetPath_CommentHistory: string;    // comment_log.csv
    function GetPath_CommentTxt(ID: string): string; // [ID].txtf
    function SanitizeCSVString(input: string): string;
    procedure SaveComment;        // Logic to save .txt and .csv
    procedure LoadComment;        // Logic to load .txt
  public
    //procedure LoadCommentForCurrentImg;
  end;
...
const
  ...
  kTab = #9;
  // 请修改为你实际保存脚本的绝对路径
  kPythonScriptPath = '/mnt/HDD1/zhenghong/MRIcroGL_LLLLL/data1/view_patient.py';
  kTotalPatients = 96; // 48个病人 * 2次扫描 = 96 (用来做边界检查)
var
 FCurrentIndex: integer = 0; // 全局变量记录当前页码
 gPrefs: TPrefs;
...
// =============================================================================
//  MASK LOADING LOGIC (Integrated from Python Script)
// =============================================================================

procedure TGLForm1.BtnLoadMasksClick(Sender: TObject);
const
  MaskFiles: array[0..5] of string = (
    'circle_10_mask.nii.gz',
    'circle_15_mask.nii.gz',
    'circle_22.5_mask.nii.gz',
    'sphere_10_mask.nii.gz',
    'sphere_15_mask.nii.gz',
    'sphere_22.5_mask.nii.gz'
  );
  MaskColors: array[0..5] of string = (
    '1red', '2green', '3blue', '6warm', '7cool', 'mako'
  );
var
  entry: TPatientEntry;
  Data1Pos, LutIndex: integer; // 新增 LutIndex
  DataRoot, ResultFolderName, CaseResultDir, MaskPath, LutPath, ColorName: string;
  i, LayerIdx: integer;
  vol: TNIfTI;
begin
  // 1. 安全检查
  if (FPatientList = nil) or (FCurrentIndex < 0) or (FCurrentIndex >= FPatientList.Count) then exit;
  entry := TPatientEntry(FPatientList[FCurrentIndex]);

  // 2. 路径解析
  Data1Pos := Pos('data1', entry.PTPath);
  if Data1Pos = 0 then begin
    Data1Pos := Pos('data1', FDataPath);
    if Data1Pos = 0 then begin
       ShowMessage('Error: Current path does not contain "data1".');
       Exit;
    end;
    DataRoot := IncludeTrailingPathDelimiter(Copy(FDataPath, 1, Data1Pos + 4));
  end else begin
    DataRoot := IncludeTrailingPathDelimiter(Copy(entry.PTPath, 1, Data1Pos + 4));
  end;

  if Pos('yCT', entry.PTPath) > 0 then
    ResultFolderName := 'RPA-result-yct-right_lobe3'
  else
    ResultFolderName := 'RPA-result-zct-right_lobe3';

  CaseResultDir := DataRoot + ResultFolderName + PathDelim + 'processed_mask' + PathDelim + entry.ID;

  if not DirectoryExists(CaseResultDir) then begin
    ShowMessage('Mask directory not found:' + sLineBreak + CaseResultDir);
    Exit;
  end;

  // 3. 循环加载 Overlay
  for i := 0 to High(MaskFiles) do begin
    MaskPath := CaseResultDir + PathDelim + MaskFiles[i];
    ShowMessage(MaskPath);
    ColorName := MaskColors[i];

    // 构建颜色表的完整路径
    LutPath := ResourceDir + PathDelim + 'lut' + PathDelim + ColorName + '.clut';

    // 【关键修改】获取颜色在下拉菜单中的正确索引
    // 如果传入 0，会被系统误认为是 Grayscale
    LutIndex := LayerColorDrop.Items.IndexOf(ColorName);

    // 如果找不到该颜色名，LutIndex 会是 -1，这对 SetDisplayColorScheme 也是安全的（代表自定义颜色）
    // 但为了保险，我们通常希望它能对应上

    if FileExists(MaskPath) then begin
      AddLayer(MaskPath);
      LayerIdx := vols.NumLayers - 1;

      if vols.Layer(LayerIdx, vol) then begin

        if FileExists(LutPath) then
           // 【关键修改】这里传入 LutIndex 而不是 0
           vol.SetDisplayColorScheme(LutPath, LutIndex)
        else
           vol.SetDisplayColorScheme('Red', -1); // 找不到颜色表时的回退

        vol.OpacityPercent := 100;
        vol.CX.NeedsUpdate := true;
      end;
    end;
  end;
...
// 1. String Sanitizer (Fixes your specific error)
function TGLForm1.SanitizeCSVString(input: string): string;
begin
  Result := input;
  // Remove line breaks
  ...
  // Replace comma with full-width comma to prevent CSV column breakage
  Result := StringReplace(Result, ',', '，', [rfReplaceAll]);
  Result := Trim(Result);
end;

function TGLForm1.GetPatientID(vol: TNIfTI): string;
var
  fName: string;
  p: integer;
begin
  // 1. 获取文件名: "QUADRA_HC_001_Test_CT-AC.nii.gz"
  fName := ExtractFileName(vol.Filename);

  // 2. 去除所有后缀 (.nii.gz) -> "QUADRA_HC_001_Test_CT-AC"
  while ExtractFileExt(fName) <> '' do
    fName := ChangeFileExt(fName, '');

  // 3. 找到最后一个下划线的位置
  // 假设格式固定为 ..._Test_CT-AC，我们需要去掉最后一个 "_" 及其后面的内容
  p := LastDelimiter('_', fName);
...
// 删除指定日志文件中当前病人的所有记录
procedure TGLForm1.DeleteHistoryLog(logFilePath: string; logDesc: string);
var
  Lines: TStringList;
...
begin
  ...
  if not FileExists(logFilePath) then begin
    ShowMessage(logDesc + ' file not found.');
    exit;
  end;
  ...
  try
    ...
    lineParts.StrictDelimiter := True;
    lineParts.Delimiter := ',';

    // 3. 倒序遍历删除 (必须倒序)
    for i := Lines.Count - 1 downto 0 do begin
      lineParts.DelimitedText := Lines[i];

      // 检查这一行是否有数据，且第一列是否为当前 ID
      if (lineParts.Count > 0) and (lineParts[0] = patientID) then begin
        Lines.Delete(i);
        Inc(deletedCount);
      end;
    end;
    ...
    if deletedCount > 0 then begin
      ...
      ShowMessage(Format('Deleted %d records from %s for %s', [deletedCount, logDesc, patientID]));
    end else begin
      ShowMessage('No records found for ' + patientID + ' in ' + logDesc);
    end;
...
procedure TGLForm1.DeleteScoreFromCSV;
var
  csvFile, historyFile: string;
  patientID, timestamp: string;
  Lines, lineParts: TStringList;
  i: integer;
  entry: TPatientEntry;
  found: boolean;
  tf: TextFile;
begin
  ...
  csvFile := GetPath_ScoreCurrent;

  // 2. Remove from Current Scores (scores.csv)
  if FileExists(csvFile) then begin
    Lines := TStringList.Create;
    lineParts := TStringList.Create;

    // Crucial for parsing CSVs correctly
    lineParts.StrictDelimiter := True;
    lineParts.Delimiter := ',';

    found := false;
    try
      Lines.LoadFromFile(csvFile);
      ...
      for i := Lines.Count - 1 downto 0 do begin
        ...
        if (lineParts.Count >= 1) and (lineParts[0] = patientID) then begin
          Lines.Delete(i);
          found := true;
        end;
      end;
      ...
      if found then begin
         Lines.SaveToFile(csvFile);
         ShowMessage('Score deleted for: ' + patientID);
      end else begin
         ShowMessage('No existing score found for: ' + patientID);
      end;
  ...
  // 3. Log Deletion to History (score_log.csv)
  // We record a score of "0" or a specific note to indicate the score was removed at this time.
  historyFile := GetPath_ScoreHistory;
  timestamp := FormatDateTime('yyyy-mm-dd hh:nn:ss', Now);

  try
    AssignFile(tf, historyFile);
    if FileExists(historyFile) then Append(tf) else Rewrite(tf);
    // Format: ID, Time, Score (0 = Deleted)
    Writeln(tf, patientID + ',' + timestamp + ',0');
    CloseFile(tf);
  except
    // Silent failure for log writing to prevent crashing main workflow
  end;
...
// =============================================================================
//  HISTORY VIEWING LOGIC (New)
// =============================================================================

// 1. 显示当前病人的分数历史
procedure TGLForm1.ShowHistoryLog;
var
  logFile, patientID, displayMsg: string;
  Lines, HistoryFound, lineParts: TStringList;
  i: integer;
  entry: TPatientEntry;
begin
  // 获取当前病人信息
  ...
  logFile := GetPath_ScoreHistory; // 获取 score_log.csv 路径

  if not FileExists(logFile) then begin
    ShowMessage('No score history found.');
    exit;
  end;

  Lines := TStringList.Create;
  lineParts := TStringList.Create;
  HistoryFound := TStringList.Create;
  ...
  try
    ...
    for i := 0 to Lines.Count - 1 do begin
      lineParts.DelimitedText := Lines[i]; // 使用 DelimitedText 自动处理逗号

      // 格式: ID, Time, Score (共3列)
      if (lineParts.Count >= 3) and (lineParts[0] = patientID) then begin
        // 格式化显示: [时间] Score: 分数
        displayMsg := Format('[%s] Score: %s', [lineParts[1], lineParts[2]]);
        HistoryFound.Add(displayMsg);
      end;
    end;

    if HistoryFound.Count > 0 then
      ShowMessage('Score History for ' + patientID + ':' + sLineBreak + sLineBreak + HistoryFound.Text)
    else
      ShowMessage('No history found for ' + patientID);
...
// 2. 显示当前病人的评论历史
procedure TGLForm1.ShowCommentHistory;
var
  logFile, patientID, displayMsg: string;
  Lines, HistoryFound, lineParts: TStringList;
  i: integer;
...
begin
  ...
  logFile := GetPath_CommentHistory; // 获取 comment_log.csv 路径

  if not FileExists(logFile) then begin
    ShowMessage('No comment history found.');
    exit;
  end;
  ...
  try
    ...
    for i := 0 to Lines.Count - 1 do begin
      ...
      // 格式: ID, Time, Comment (共3列)
      if (lineParts.Count >= 3) and (lineParts[0] = patientID) then begin
        // 格式化显示: [时间]: 评论内容
        displayMsg := Format('[%s]: %s', [lineParts[1], lineParts[2]]);
        HistoryFound.Add(displayMsg);
      end;
    end;

    if HistoryFound.Count > 0 then
      ShowMessage('Comment History for ' + patientID + ':' + sLineBreak + sLineBreak + HistoryFound.Text)
    else
      ShowMessage('No comments found for ' + patientID);
...
// 辅助函数：根据分数更新界面 (Radio和CheckBox)
//procedure TGLForm1.UpdateScoreUI(score: integer);
//begin
//  // 如果 score 是 0 (未评分)，下面所有等式都为 False，按钮会自动取消选中
//  RadioScore1.Checked := (score = 1);
//  RadioScore2.Checked := (score = 2);
//  RadioScore3.Checked := (score = 3);
//  RadioScore4.Checked := (score = 4);
//  RadioScore5.Checked := (score = 5);
//
//  // 更新 CheckBox 状态：只有 1-5 分时才算 "Scored"
//  CheckIsScored.Checked := (score >= 1) and (score <= 5);
//end;
...
begin
  // 逻辑：如果 score 为 0 (表示未评分)，我们默认显示为 1
  if score = 0 then
    displayScore := 1
  else
    displayScore := score;

  // 更新单选框 (根据 displayScore)
  RadioScore1.Checked := (displayScore = 1);
  RadioScore2.Checked := (displayScore = 2);
  RadioScore3.Checked := (displayScore = 3);
  RadioScore4.Checked := (displayScore = 4);
  RadioScore5.Checked := (displayScore = 5);

  // 更新 CheckBox 状态 (根据原始 score)
  // 只有当原始分数为 1-5 时，才算 "已评分" (Scored)
  CheckIsScored.Checked := (score >= 1) and (score <= 5);
end;

procedure TGLForm1.SaveScoreToCSV;
var
  entry: TPatientEntry;
  patientID, timestamp, scoreStr: string;
  newScore, i: integer;
  found: boolean;
  csvFile: string;
  Lines, lineParts: TStringList;
...
begin
  // 1. 获取 UI 分数
  newScore := 0;
  if RadioScore1.Checked then newScore := 1
  else if RadioScore2.Checked then newScore := 2
  else if RadioScore3.Checked then newScore := 3
  else if RadioScore4.Checked then newScore := 4
  else if RadioScore5.Checked then newScore := 5;

  if newScore = 0 then begin
    ShowMessage('Please select a score first.');
    exit;
  end;

  // 2. 获取 ID (直接从 FPatientList 获取更安全)
  if (FPatientList = nil) or (FCurrentIndex < 0) or (FCurrentIndex >= FPatientList.Count) then exit;
  entry := TPatientEntry(FPatientList[FCurrentIndex]);
  patientID := entry.ID;
  timestamp := FormatDateTime('yyyy-mm-dd hh:nn:ss', Now);
  scoreStr := IntToStr(newScore);

  // 3. 更新 scores.csv (覆盖模式)
  csvFile := GetPath_ScoreCurrent;
  Lines := TStringList.Create;
  lineParts := TStringList.Create;
  lineParts.StrictDelimiter := True;
  ...
  try
    if FileExists(csvFile) then Lines.LoadFromFile(csvFile);

    for i := 0 to Lines.Count - 1 do begin
      lineParts.DelimitedText := Lines[i];
      if (lineParts.Count >= 1) and (lineParts[0] = patientID) then begin
        Lines[i] := patientID + ',' + timestamp + ',' + scoreStr;
        found := true;
        break;
      end;
    end;
    ...
    if not found then
      Lines.Add(patientID + ',' + timestamp + ',' + scoreStr);
  ...
  // 4. 追加到 score_log.csv (历史记录)
  csvFile := GetPath_ScoreHistory;
  try
    AssignFile(tf, csvFile);
    if FileExists(csvFile) then Append(tf) else Rewrite(tf);
    Writeln(tf, patientID + ',' + timestamp + ',' + scoreStr);
    CloseFile(tf);
  except
  end;
...
procedure TGLForm1.LoadScoreFromCSV;
var
  csvFile, patientID: string;
  Lines, lineParts: TStringList;
...
begin
  ...
  csvFile := GetPath_ScoreCurrent; // 读取 scores.csv
  ...
  if FileExists(csvFile) then begin
    ...
    try
      ...
      for i := Lines.Count - 1 downto 0 do begin
        lineParts.DelimitedText := Lines[i];
        if (lineParts.Count >= 3) and (lineParts[0] = patientID) then begin
          currentScore := StrToIntDef(lineParts[2], 0);
          break;
        end;
      end;
    finally
...
procedure TGLForm1.ClearPatientList;
var
  i: integer;
begin
  if FPatientList <> nil then begin
    for i := 0 to FPatientList.Count - 1 do
      TPatientEntry(FPatientList[i]).Free;
    FPatientList.Clear;
  end else begin
    FPatientList := TList.Create;
  end;
end;

procedure TGLForm1.InitFileLists;
var
  chosenDir, data1Root: string;
  parentDir, folderName: string;
  info: TSearchRec;
  fName, fID: string;
  i: integer;
  entry: TPatientEntry;
  foundEntry: TPatientEntry;
  isPT, isCT: boolean;
  FolderDefaultType: Integer;
  Reply: Integer;
  rawName: string;
  HasImages: boolean;
...
begin
  // 设置默认起始路径
  data1Root := ExpandFileName(ExtractFilePath(Application.ExeName) + '..' + PathDelim + 'data1');
  if not DirectoryExists(data1Root) then
    data1Root := ExtractFilePath(Application.ExeName);

  UserWantsToRetry := True;

  // --- 开始循环：直到找到图像或用户放弃 ---
  while UserWantsToRetry do begin
    // 1. 初始化清理
    ClearPatientList;
    FCurrentIndex := 0;
    FolderDefaultType := 0;
    ...
    // 2. 选择文件夹
    if not SelectDirectory('Select Data Folder containing .nii.gz files', data1Root, chosenDir) then
    begin
      // 用户在选择框点击了“取消”，直接退出函数
      Exit;
    end;

    FDataPath := IncludeTrailingPathDelimiter(chosenDir);

    // 3. 计算日志路径
    folderName := ExtractFileName(ExcludeTrailingPathDelimiter(FDataPath));
    parentDir := ExtractFilePath(ExcludeTrailingPathDelimiter(FDataPath));
    FLogPath := parentDir + 'Logs' + PathDelim + folderName + '_Logs' + PathDelim;

    // 4. 遍历文件
    if FindFirst(FDataPath + '*.nii*', faAnyFile, info) = 0 then
    begin
      try
        repeat
          if (info.Name = '.') or (info.Name = '..') then Continue;

          isPT := False; isCT := False; fID := '';

          // 获取基础文件名
          rawName := ExtractFileName(info.Name);
          while (ExtractFileExt(rawName) = '.gz') or (ExtractFileExt(rawName) = '.nii') do
            rawName := ChangeFileExt(rawName, '');

          // 识别逻辑
          if EndsText('_PT-SUV', rawName) then begin
            isPT := True; fID := Copy(rawName, 1, Length(rawName) - 7);
          end
          else if EndsText('_CT-AC', rawName) then begin
            isCT := True; fID := Copy(rawName, 1, Length(rawName) - 6);
          end
          else begin
            // 非标准命名：询问用户（只问一次）
            if FolderDefaultType = 0 then begin
              Reply := MessageDlg('Unknown File Type',
                Format('File "%s" does not match standard naming.' + sLineBreak +
                       'How should NON-STANDARD files in this folder be treated?' + sLineBreak + sLineBreak +
                       '[Yes] = Treat all as CT' + sLineBreak +
                       '[No]  = Treat all as PET' + sLineBreak +
                       '[Cancel] = Skip non-standard files', [info.Name]),
                mtConfirmation, [mbYes, mbNo, mbCancel], 0);

              if Reply = mrYes then FolderDefaultType := 1      // CT
              else if Reply = mrNo then FolderDefaultType := 2  // PET
              else FolderDefaultType := -1;                     // Skip
            end;

            if FolderDefaultType = 1 then begin isCT := True; fID := rawName; end
            else if FolderDefaultType = 2 then begin isPT := True; fID := rawName; end
            else Continue; // 跳过
          end;
          ...
          if fID <> '' then begin
            foundEntry := nil;
            // 查找已存在的 ID
            for i := 0 to FPatientList.Count - 1 do begin
              if TPatientEntry(FPatientList[i]).ID = fID then begin
                foundEntry := TPatientEntry(FPatientList[i]); Break;
              end;
            end;

            // 创建新条目
            if foundEntry = nil then begin
              foundEntry := TPatientEntry.Create;
              foundEntry.ID := fID; foundEntry.CTPath := ''; foundEntry.PTPath := '';
              FPatientList.Add(foundEntry);
            end;

            // 填充路径
            if isPT then foundEntry.PTPath := FDataPath + info.Name
            else if isCT then foundEntry.CTPath := FDataPath + info.Name;
          end;
    ...
    if FPatientList.Count > 0 then begin
      // --- 成功找到图像 ---
      if SpinJumpIndex <> nil then begin
        SpinJumpIndex.MaxValue := FPatientList.Count;
        SpinJumpIndex.Value := 1;
      end;

      LoadCurrentPair; // 加载第一张图
      UserWantsToRetry := False; // 退出循环
      if not DirectoryExists(FLogPath) then ForceDirectories(FLogPath);
    end
    else begin
      // --- 没找到图像：给予选项 ---
      Reply := MessageDlg('No Images Found',
        'No NIfTI images were found in the selected folder:' + sLineBreak +
        FDataPath + sLineBreak + sLineBreak +
      ...
      if Reply = mrYes then begin
        // 用户选择重试：循环继续，重新弹出选择框
        UserWantsToRetry := True;
        // 小优化：下次打开对话框时，可以指向上一次选的父目录，或者保持 data1Root
        // data1Root := parentDir;
      end else begin
        // 用户选择退出/取消：退出循环
        UserWantsToRetry := False;
        // 可选：如果没数据就关闭程序，取消注释下面这行
...
function TGLForm1.GetPath_ScoreCurrent: string;
begin
  Result := FLogPath + 'scores.csv'; // 对应需求
end;

function TGLForm1.GetPath_ScoreHistory: string;
begin
  Result := FLogPath + 'score_log.csv'; // 对应需求
end;

function TGLForm1.GetPath_CommentHistory: string;
begin
  Result := FLogPath + 'comment_log.csv'; // 对应需求
end;

function TGLForm1.GetPath_CommentTxt(ID: string): string;
begin
  // 每个病人单独的 txt 文件
  Result := FLogPath + ID + '.txt';
end;



// 获取存放评论的文件夹路径 (跨平台安全路径)
function GetCommentDir: string;
begin
  // Windows: C:\Users\Username\.mricrogl_comments\
  // Linux: /home/username/.mricrogl_comments/
  // result := '/mnt/HDD1/zhenghong/MRIcroGL_LLLLL/data1/comment/';
  result := ExtractFilePath(Application.ExeName) + '..' + PathDelim + 'data1' + PathDelim + 'comment' + PathDelim;
...
// 获取当前背景图像对应的评论文件完整路径
function GetCurrentCommentFile: string;
var
  vol: TNIfTI;
  imgName: string;
begin
  result := '';
  // 检查是否加载了背景图 (Layer 0)
  if (vols = nil) or (vols.NumLayers < 1) then exit;
  if not vols.Layer(0, vol) then exit;

  // 获取文件名（不包含路径）
  imgName := ExtractFileName(vol.Filename);

  // 循环去除扩展名 (比如 .nii.gz 去掉两次变为纯文件名)
  while ExtractFileExt(imgName) <> '' do
      imgName := ChangeFileExt(imgName, '');

  // 组合成: 评论目录 + 图片名 + .txt
  result := GetCommentDir + imgName + '.txt';
end;
...
procedure TGLForm1.SaveComment;
var
  entry: TPatientEntry;
  patientID, timestamp, commentText, txtFile, csvFile: string;
  tf: TextFile;
begin
  if (FPatientList = nil) or (FCurrentIndex < 0) or (FCurrentIndex >= FPatientList.Count) then exit;
  entry := TPatientEntry(FPatientList[FCurrentIndex]);
  ...
  // 1. 保存当前评论到 .txt 文件 (覆盖)
  txtFile := GetPath_CommentTxt(patientID);
  try
    MemoComment.Lines.SaveToFile(txtFile);
  except
    ShowMessage('Error saving comment text file.');
    exit;
  end;

  // 2. 追加到 comment_log.csv (历史)
  commentText := SanitizeCSVString(MemoComment.Text);
  if Trim(commentText) = '' then exit; // 如果是空的就不记日志了？可选。

  csvFile := GetPath_CommentHistory;
  try
    AssignFile(tf, csvFile);
    if FileExists(csvFile) then Append(tf) else Rewrite(tf);
    // 格式: ID, Time, Content
    Writeln(tf, patientID + ',' + timestamp + ',' + commentText);
    CloseFile(tf);
  except
  end;
end;

procedure TGLForm1.LoadComment;
var
  entry: TPatientEntry;
  txtFile: string;
begin
  ...
  // 从 .txt 文件加载当前评论
  txtFile := GetPath_CommentTxt(entry.ID);

  if FileExists(txtFile) then begin
    try
      MemoComment.Lines.LoadFromFile(txtFile);
    except
      // 忽略读取错误
    end;
  end;
...
procedure TGLForm1.BtnClearCommentHistChange(Sender: TObject);
begin
  if MessageDlg('Confirm', 'Delete ALL comment history for this patient?', mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    DeleteHistoryLog(GetPath_CommentHistory, 'Comment Log');
  end;
end;

procedure TGLForm1.BtnClearScoreHistClick(Sender: TObject);
begin
  if MessageDlg('Confirm', 'Delete ALL score history for this patient?', mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    DeleteHistoryLog(GetPath_ScoreHistory, 'Score Log');
  end;
end;

procedure TGLForm1.BtnCommentHistoryClick(Sender: TObject);
begin
  ShowCommentHistory;
end;

procedure TGLForm1.BtnDeleteScoreClick(Sender: TObject);
begin
  // 可选：增加一个确认对话框，防止误删
  if MessageDlg('Confirm', 'Delete score for this image?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  begin
    DeleteScoreFromCSV;
  end;
end;

procedure TGLForm1.BtnHistoryClick(Sender: TObject);
begin
  ShowHistoryLog;
end;

procedure TGLForm1.BtnJumpToClick(Sender: TObject);
begin
  // 用户输入的是 1-based，我们转成 0-based
  FCurrentIndex := SpinJumpIndex.Value - 1;
  LoadCurrentPair;
end;

procedure TGLForm1.BtnNextPatientClick(Sender: TObject);
begin
  Inc(FCurrentIndex); // 索引 +1
  LoadCurrentPair;
end;

procedure TGLForm1.BtnPrevPatientClick(Sender: TObject);
begin
  Dec(FCurrentIndex); // 索引 -1
  LoadCurrentPair;
end;

procedure TGLForm1.BtnSaveCommentClick(Sender: TObject);
begin
  // 保存评论 (txt + log csv)
  SaveComment;
...
procedure TGLForm1.BtnSaveScoreClick(Sender: TObject);
begin
  SaveScoreToCSV;
...
procedure TGLForm1.CheckAutoRunPyChange(Sender: TObject);
begin
...
procedure TGLForm1.GroupBox2Click(Sender: TObject);
begin

end;

procedure TGLForm1.GroupBox3Click(Sender: TObject);
begin
...
begin
  case gPrefs.DisplayOrient of
      ...
      kAxCorSagOrient3_LeftNose: MPRLeftNoseMenu.Checked := true;

      //kAxCorSagOrient3_LeftNose: begin
      //     // 确保这里的菜单名和你设计器里的 Name 一致
      //     if Assigned(FindComponent('MPRLeftNoseMenu')) then
      //        TMenuItem(FindComponent('MPRLeftNoseMenu')).Checked := true;
      // end;
  end;
end;
...
begin
     if (setMenuChecked) then
        SetDisplayCheck();
     SliceBox.Visible := (gPrefs.DisplayOrient <= kAxCorSagOrient3) or (gPrefs.DisplayOrient = kAxCorSagOrient3_LeftNose);
...
procedure TGLForm1.TimerFixPropsTimer(Sender: TObject);
begin
...
begin
  ...
  DisplayCombo.Items.Add('Startup: Render');                  // Index 0
  DisplayCombo.Items.Add('Startup: Axial');                   // Index 1
  DisplayCombo.Items.Add('Startup: Coronal');                 // Index 2
  DisplayCombo.Items.Add('Startup: Sagittal');                // Index 3
  DisplayCombo.Items.Add('Startup: Sagittal (nose left)');    // Index 4
  DisplayCombo.Items.Add('Startup: Multi-Planar (A+C+S)');    // Index 5
  DisplayCombo.Items.Add('Startup: Multi-Planar (A+C+S+R)');  // Index 6
  DisplayCombo.Items.Add('Startup: Multi-Planar (Left Nose)');// Index 7 【新增】
  ...
  case (gPrefs.StartupDisplayOrient) of
    ...
    kAxCorSagOrient3_LeftNose: v := 7; // 【新增】 对应 Index 7
    else v := 5; // kAxCorSagOrient3
  end;
  ...
  case DisplayCombo.ItemIndex of
    ...
    7: gPrefs.StartupDisplayOrient := kAxCorSagOrient3_LeftNose; // 【新增】 对应 Index 7
    else gPrefs.StartupDisplayOrient := kAxCorSagOrient3; // Index 5
  end;
...
procedure TGLForm1.MemoCommentChange(Sender: TObject);
begin
...
procedure TGLForm1.MosRowLabelClick(Sender: TObject);
begin
...
procedure TGLForm1.RadioScore2Change(Sender: TObject);
begin
...
var
   ...
   f: single = 1.0;
   // 【新增变量】
   fracXYZ, oldCenter, newCenter: TVec3;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
procedure TGLForm1.LoadCurrentPair;
var
  entry: TPatientEntry;
  v0, v1: TNIfTI;
  invertedLUT: string;
  ...
  // --- 内部辅助函数：统一设置 PET 样式 ---
  procedure ApplyPetStyle(vol: TNIfTI);
  begin
    // 1. 设置颜色表 (Inverted 或 Red)
    if FileExists(invertedLUT) then
       vol.SetDisplayColorScheme(invertedLUT, 0)
    else
       vol.SetDisplayColorScheme('Grayscale', 0); // 回退方案

    // 2. 设置 SUV 显示范围
    vol.SetDisplayMinMax(-0.2, 5.0);
...
begin
  invertedLUT := ResourceDir + PathDelim + 'lut' + PathDelim + 'inverted.clut';

  // 1. 安全检查
  if (FPatientList = nil) or (FPatientList.Count = 0) then exit;

  // 2. 索引循环处理
  if FCurrentIndex < 0 then FCurrentIndex := FPatientList.Count - 1;
  if FCurrentIndex >= FPatientList.Count then FCurrentIndex := 0;

  // 3. 获取当前病人的数据对象
  entry := TPatientEntry(FPatientList[FCurrentIndex]);

  // 4. 更新 UI 索引显示
  if SpinJumpIndex <> nil then SpinJumpIndex.Value := FCurrentIndex + 1;

  // =========================================================================
  //  图像加载逻辑
  // =========================================================================

  // 情况 A: 存在 CT 文件 -> CT 做背景，PET 做叠加
  if (entry.CTPath <> '') and FileExists(entry.CTPath) then begin

    // A1. 加载 CT 作为背景
    if not AddBackground(entry.CTPath) then begin
       ShowMessage('Failed to load: ' + entry.CTPath);
       exit;
    end;

    // 设置 CT 样式 (Layer 0)
    if vols.Layer(0, v0) then begin
       v0.SetDisplayColorScheme('Grayscale', 0);
       v0.SetDisplayMinMax(-800.0, 150.0); // 软组织窗
       v0.OpacityPercent := 0;
    end;

    // A2. 如果存在 PET，加载为 Overlay (Layer 1)
    if (entry.PTPath <> '') and FileExists(entry.PTPath) then begin
       if AddLayer(entry.PTPath) then begin
          Application.ProcessMessages; // 确保加载完成
          if vols.Layer(1, v1) then begin
             ApplyPetStyle(v1); // <--- 应用 PET 样式到 Layer 1
          end;
       end;
    end;

  end
  // 情况 B: 没有 CT，只有 PET -> PET 做背景
  else if (entry.PTPath <> '') and FileExists(entry.PTPath) then begin

    // B1. 加载 PET 作为背景 (Layer 0)
    if not AddBackground(entry.PTPath) then begin
       ShowMessage('Failed to load: ' + entry.PTPath);
       exit;
    end;

    // B2. 设置 PET 样式 (Layer 0)
    if vols.Layer(0, v0) then begin
       ApplyPetStyle(v0); // <--- 应用 PET 样式到 Layer 0
    end;
  ...
  // 刷新左侧设置面板，确保界面显示的数值与代码设置一致
  UpdateLayerBox(false);
  UpdateTimer.Enabled := true;

  // 重置十字准线
  SetXHairPosition(2,150,1500);
  ViewGPU1.Invalidate;

  // 加载 CSV 数据 (分数和评论)
  // 在最后调用新的加载函数
  LoadScoreFromCSV; // 从 scores.csv 读取
  LoadComment;      // 从 [ID].txt 读取

  Self.Caption := 'Reviewing: ' + entry.ID + ' (' + IntToStr(FCurrentIndex+1) + '/' + IntToStr(FPatientList.Count) + ')';
end;
...
procedure TGLForm1.Label1Click(Sender: TObject);
begin
...
procedure TGLForm1.LayerBoxClick(Sender: TObject);
begin
...
begin
  ...
  // 清理病人列表
  ClearPatientList;
  if FPatientList <> nil then FPatientList.Free;
